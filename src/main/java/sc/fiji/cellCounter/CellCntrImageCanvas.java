...
public class CellCntrImageCanvas extends ImageCanvas {
	...
	public CellCntrImageCanvas(final ImagePlus img,
		...
		final Overlay displayList)
	{
		super(img);
		this.img = img;
		this.typeVector = typeVector;
		this.cc = cc;
		// Changed setDisplayList to setOverlay due to deprecation of ImageCanvas - Ian Oh
		if (displayList != null) {
	...
	public ImagePlus imageWithMarkers() {
		...
		final Overlay displayList = getOverlay();
		if (displayList != null && displayList.size() == 1) {
			final Roi roi = displayList.get(0);
